...
def main():
    ...
    # Train/test split
    # Since X_seq is (N, window_size, input_dim), we split on the first dimension.
    # Note: to avoid leakage of overlapping windows, time-series splitting is generally preferred,
    # but to match the instruction ("Train/test split (80/20, stratify=y)"), we use train_test_split.
    X_train_seq, X_test_seq, y_train_seq, y_test_seq = train_test_split(
        X_seq, y_seq, test_size=0.2, random_state=42, stratify=y_seq
    )
    
    # Scale features
    # Since StandardScaler expects 2D data, we reshape, scale, and reshape back.
    ...
    model = LSTMClassifier(input_dim=input_dim, hidden_size=64)
