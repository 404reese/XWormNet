...
def render():
    ...
    with col1:
        ...
        bars = ax.bar(df["Model"], df["F1"], color=["#1f77b4", "#ff7f0e"][:len(df)])
    ...
    with col2:
        ...
        if "Latency (ms)" in df.columns:
            bars = ax.bar(df["Model"], df["Latency (ms)"], color=["#1f77b4", "#ff7f0e"][:len(df)])
    ...
    # REAL insight
    st.info("💡 Key Insight: LNN is designed for faster execution and smaller footprint for IoT edge deployment, while RF may have a higher F1 score suitable for enterprise servers.")
